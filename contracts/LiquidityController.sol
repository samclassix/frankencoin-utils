...
contract LiquidityController is IERC721Receiver, AccessControl {
	...
	modifier onlyAdminsOrExecutors() {
		require(hasRole(ADMIN_ROLE, msg.sender) == true || hasRole(EXECUTOR_ROLE, msg.sender) == true, 'No AdminOrExecutor');
		_;
	}
	...
	constructor(INonfungiblePositionManager _nonfungiblePositionManager, address _admin, address _exec) {
		...
		_grantRole(EXECUTOR_ROLE, _exec);
	}
	...
	function _createDeposit(uint256 tokenId) internal {
		...
		deposits[tokenId] = Deposit({owner: address(this), liquidity: liquidity, token0: token0, token1: token1, poolFee: fee});
	}
	...
	// ---------------------------------------------------------------------------------------
	// transfer from msg.sender for a specific deposit type, -> needs allowance
	function transferForDeposit(uint256 tokenId, uint256 amount0, uint256 amount1) external onlyAdmins {
		TransferHelper.safeTransferFrom(deposits[tokenId].token0, msg.sender, amount0);
		TransferHelper.safeTransferFrom(deposits[tokenId].token1, msg.sender, amount1);
	}

	// ---------------------------------------------------------------------------------------
	// set the allowance for any two tokens to the manager
	function setAllowanceForManager(address token0, address token1, uint256 amount0, uint256 amount1) external onlyAdmins {
	...
	function decreaseLiquidity(
		...
		require(deposits[tokenId].liquidity >= liquidity, 'not enough liquidity');
